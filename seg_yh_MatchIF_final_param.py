...
class Segmentation:
    def __init__(self, image, output_dir, ext, outlines):
        self.image_name = str(image)
        self.output_dir = output_dir
        self.ext = ext
        self.outlines = outlines
        self.I = io.imread(image).astype(np.uint8)
    
    def run(self):
        print "Segmentation Running...."
        
        R = self.I[:,:,0]
        Ilab = cv2.cvtColor(self.I, cv2.COLOR_RGB2LAB)
        Igray = cv2.cvtColor(Ilab, cv2.COLOR_RGB2GRAY)
        
        Ich = Igray
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        Ich = clahe.apply(Igray)  # Or cv2.equalizeHist(Igray)
        ...
        ## Final mask (morphological cleaning)
        ...
        ## Save 
        I_bd = mark_boundaries(self.I, mask_final, color=(0, 1,1))
        io.imsave(self.outlines+self.ext, I_bd) 
        io.imsave(self.output_dir+self.ext, mask_final, cmap="gray")   
...
img = Segmentation(options.image, options.output_dir, options.ext, options.outlines)
img.run()  
